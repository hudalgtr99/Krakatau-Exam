...
function F_getDirTable($dir, $selected = '', $params = '', $rootdir = K_PATH_CACHE, $authdirs = '')
{
    ...
    $out .= '<div class="overflow-auto">'; // html string to be returned
    $out .= '<table class="table">'.K_NEWLINE;
    // header
    $out .= '<tr>';
    $out .= '<th>'.$l['w_name'].'</th>';
    $out .= '<th>'.$l['w_size'].'</th>';
    ...
    foreach ($data['dirs'] as $file) {
        ...
        if ($file == $usrdir) {
            ...
            $out .= '<tr class="text-monospace">';
			$folder = '<i class="text-info fa fa-folder"></i>';
        }
        $out .= '<td>'.$folder.'&nbsp;<a href="'.$_SERVER['SCRIPT_NAME'].'?d='.$current_dir.'&amp;v=1'.$params.'" title="'.$l['w_change_dir'].'">'.$info['basename'].'</a></td>';
        $out .= '<td>'.F_formatFileSize($info['size']).'</td>';
    ...
    foreach ($data['files'] as $file) {
        ...
        if (isset($info['extension']) and in_array(strtolower($info['extension']), $allowed_extensions) and (substr($info['basename'], 0, 6) != 'latex_')) {
            ...
            if ($info['basename'] == $selected) {
                $out .= '<tr class="bg-light font-weight-bold text-monospace">';
            } else {
                $out .= '<tr class="text-monospace">';
            }
...
function F_getDirVisualTable($dir, $selected = '', $params = '', $rootdir = K_PATH_CACHE, $authdirs = '')
{
    ...
    $data = F_getDirFiles($dir, $rootdir, $authdirs);
    // dirs
	$out .= '<div class="d-flex flex-wrap justify-content-start">';
    foreach ($data['dirs'] as $file) {
        $info = F_getFileInfo($file);
        $current_dir = urlencode($dir.$info['basename'].'/');
        
        $out .= '<div class="d-flex flex-column-reverse justify-content-between p-1 m-1 border" style="width:150px">';
    ...
    foreach ($data['files'] as $file) {
        ...
        if (isset($info['extension']) and in_array(strtolower($info['extension']), $allowed_extensions) and (substr($info['basename'], 0, 6) != 'latex_')) {
			...
			$out .= '<div class="d-flex flex-column justify-content-between p-1 m-1 border flex-column-reverse" style="width:150px">';
			$out .= '<div class="align-self-center">';
            $filename = $info['basename'];
            if (strlen($filename) > 20) {
