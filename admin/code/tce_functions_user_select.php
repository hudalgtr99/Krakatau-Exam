    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
            ...
            if ($_SESSION['session_user_level'] >= K_AUTH_DELETE_USERS) {
                ...
                F_submit_button_confirm('delete', $l['w_delete'], $l['h_delete'], 'onclick="return confirm(\''.$l['m_delete_confirm'].'\')" class="d-block btn btn-danger btn-block"');
                // echo '</div>'.K_NEWLINE;
                echo '</li>'.K_NEWLINE;
            }
            if ($_SESSION['session_user_level'] >= K_AUTH_ADMIN_GROUPS) {
				...
				echo '<div class="mr-2"></div>'.K_NEWLINE;
				// echo '<div class="input-group-prepend">'.K_NEWLINE;
                F_submit_button_alt('addgroup', $l['w_add'], $l['w_add'], 'mr-2 btn btn-success');
				// echo '</div>'.K_NEWLINE;
                if ($_SESSION['session_user_level'] >= K_AUTH_DELETE_GROUPS) {
                    F_submit_button_confirm('delgroup', $l['w_delete'], $l['h_delete'], 'class="btn btn-danger" onclick="return confirm(\''.$l['m_delete_confirm'].'\')"');
                }
				echo '</div>'.K_NEWLINE;
                echo '</li>'.K_NEWLINE;
                if ($_SESSION['session_user_level'] >= K_AUTH_MOVE_GROUPS) {
                    ...
                    if ($l['a_meta_dir'] == 'rtl') {
                        $arr = '<i class="fa fa-arrow-circle-left"></i>';
                    } else {
                        $arr = '<i class="fa fa-arrow-circle-right"></i>';
                    }
					
                    echo F_user_group_select('from_group_id');
					echo '<div class="d-flex align-items-center">'.K_NEWLINE;
                    echo $arr;
					echo '</div>'.K_NEWLINE;
                    echo F_user_group_select('to_group_id');
					echo '<div class="mr-2"></div>'.K_NEWLINE;
                    F_submit_button_alt('move', $l['w_move'], $l['w_move'], 'btn btn-primary');
					echo '</div>'.K_NEWLINE;
                    echo '</li>'.K_NEWLINE;
                }
            }
