...
{
    require_once('../config/tce_config_upload.php');
...
<?php
    $out .= '<div class="tblcont"><table class="userselect">'.K_NEWLINE;
    // header
    $out .= '<tr>';
    $out .= '<th>'.$l['w_name'].'</th>';
    $out .= '<th>'.$l['w_size'].'</th>';
    ...
    foreach ($data['files'] as $file) {
        ...
        if (isset($info['extension']) and in_array(strtolower($info['extension']), $allowed_extensions) and (substr($info['basename'], 0, 6) != 'latex_')) {
            ...
            $out .= '<td style="text-align:left;">'.F_formatFileSize($info['size']).'</td>';
...
function F_getDirVisualTable($dir, $selected = '', $params = '', $rootdir = K_PATH_CACHE, $authdirs = '')
{
    ...
    foreach ($data['dirs'] as $file) {
        ...
        $out .= '<tr style="height:16px;font-family:monospace;font-weight:bold;color:white;"><th>';
    ...
    foreach ($data['files'] as $file) {
        ...
        if (isset($info['extension']) and in_array(strtolower($info['extension']), $allowed_extensions) and (substr($info['basename'], 0, 6) != 'latex_')) {
            ...
            $out .= '<tr style="height:16px;font-family:monospace;color:white;"><th>';
            $filename = $info['basename'];
            if (strlen($filename) > 20) {
