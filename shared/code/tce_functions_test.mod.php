                ...
                if (($test_status >= 4) and F_getBoolean($m['test_results_to_users'])) {
                    ...
                    if (isset($usrtestdata['user_score']) and isset($usrtestdata['test_score_threshold']) and ($usrtestdata['test_score_threshold'] > 0)) {
                        if ($usrtestdata['user_score'] >= $usrtestdata['test_score_threshold']) {
                            $str .= ' style="background-color:#e8fce8;"';
                            $passmsg = ' - '.$l['w_passed'];
                        } else {
                            $str .= ' style="background-color:#FFEEEE;"';
                ...
                if (!$expired) {
                    switch ($test_status) {
                        case 0: { // 0 = the test generation process is started but not completed
                            ...
                            $str .= '?testid='.$m['test_id'].'" title="'.$l['h_execute'].'" class="buttongreen"><span class="icon-chevron-circle-right" onclick="clearUnsure()"></span> '.$l['w_execute'].'</a>';
                            break;
                        }
    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
			...
			if(K_ENABLE_HTML){
				...
				$qblock_arr['qdesc'] = html_entity_decode($m['question_description']).K_NEWLINE;
				// print_r($qblock_arr);
			}else{
            ...
            if ($m['question_type'] == 3) {
                ...
                $sqla = 'SELECT *
					...
					WHERE logansw_answer_id=answer_id
						AND logansw_testlog_id='.$testlog_id.'
					ORDER BY logansw_order';
					$qblock_arr['autonext'] = $m['question_auto_next'];
                        ...
                        switch ($m['question_type']) {
                            case 1: {
								...
								if(K_ENABLE_HTML){
									...
									$qblock_arr['ansdesc'][$anspos] = html_entity_decode($ma['answer_description']);
								}else{
                ...
                if ($m['question_type'] == 1) {
                    ...
                    // $str .= $l['m_unanswered'];
					$qblock_arr['ansdesc'][0] = $l['m_unanswered'];
                    // $str .= '</label>';
                    // $str .= '</li>'.K_NEWLINE;
                }
...
function F_questionsMenu($testdata, $testuser_id, $testlog_id = 0, $disable = false)
{
    ...
    if ($r = F_db_query($sql, $db)) {
        ...
        while ($m = F_db_fetch_array($r)) {
            ...
            if ($m['testlog_id'] != $testlog_id) {
				...
				$str .= '<button type="button" name="'.$m['testlog_id'].'" id="'.$m['testlog_id'].'" value="'.$i.'" onclick="if(navigator.onLine){saveAnswerAjax(1000,this)}else{pageOffline()}"';
                if ($testlog_id_last == $testlog_id) {
                    $testlog_id_next = $m['testlog_id'];
                }
            } else {
	...
	if(!isset($_POST['question-block'])){
		...
		$navlink .= '<button id="prevbtn" onclick="if(navigator.onLine){saveAnswerAjax(-1,this)}else{pageOffline()}" type="button" title="&#10094; '.$l['w_previous'].'"';
		if (($testlog_id_prev <= 0) or ($testlog_id_prev > $testlog_id)) {
		...
		$navlink .= '<button id="nextbtn" onclick="if(navigator.onLine){saveAnswerAjax(1,this)}else{pageOffline()}" type="button" title="'.$l['w_next'].' &#10095;"><span class="hidden show768">'.$l['w_next'].'</span> &#10095;</button>';
		$navlink .= '</div>';
		$navlink .= '<div class="d-flex jc-c ai-unset mt-10">';
		$navlink .= '<button id="relbtn" onclick="if(navigator.onLine){saveAnswerAjax(0,this)}else{pageOffline()}" type="button" title="'.$l['w_save'].'">'.$l['w_save'].'</button>';
