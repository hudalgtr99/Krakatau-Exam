...
* return $data string containing HTML table.
*/
function F_printUserTestStat($testuser_id,$test_id)
{
    require_once('../config/tce_config.php');
    require_once('../../shared/code/tce_functions_tcecode.php');
    global $db, $l;
	...
	$noanswer_hidden = '';
    $noanswer_disabled = '';
    if (!F_getBoolean($test_data['test_noanswer_enabled'])) {
	...
	echo '<script>';
	if(TRIGSS_CACHE_ALL){	
		echo 'var arrTestLogIDfromServer = [];'.K_NEWLINE;
	}
    if ($r = F_db_query($sql, $db)) {
		$a = -1;
        while ($m = F_db_fetch_array($r)) {
			$a++;
            // $ret .= 'testlogid = '.$m['testlog_id'].'<br/>';
			$qblock_arr['testlogid'] = intval($m['testlog_id']);
            // $ret .= 'qtype = '.$m['question_type'].'<br/>';
			$qblock_arr['qtype'] = $m['question_type'];
            // $ret .= 'content = ';
			$qblock_arr['content'] = '';
            // display question description
			if ($m['question_type'] == 3) {
				// $ret .= '<label for="answertext">';	
			...
			// $ret .= '<div class="rowl">'.K_NEWLINE;
			$qblock_arr['content'] .= '<div class="rowl">'.K_NEWLINE;
            if ($m['question_type'] == 3){
                // TEXT
				// $ret .= '<div id="question-type-3"></div><div id="hiddenAnswerText" class="hidden"></div><textarea cols="70" rows="15" name="answertext" id="answertext" ></textarea><a class="xmlbutton buttonblue btn-outlined" name="editorLoaderBtn" id="editorLoaderBtn" onclick="loadWYSIBB()"><span class="icon-wysiwyg"></span> Muat Editor</a><div id="btn_uploadFileCont" style="display:none"><a onclick="window.open(\'tce_select_userphoto.php?frm=testform&amp;fld=answertext,\',\'mediaselect\',\'height=600,width=680,resizable=yes,menubar=no,scrollbars=yes,toolbar=no,status=no,modal=yes\');" id="btn_uploadFile" class="xmlbutton buttonblue"><span class="icon-info"></span> upload file / Sisipkan media</a>&nbsp;<a id="imgProblem" href="#imgProblem">Apabila gambar tidak tampil pada editor, silakan klik disini</a></div>';
				$qblock_arr['content'] .= '<div id="question-type-3"></div><div id="hiddenAnswerText" class="hidden"></div><textarea cols="70" rows="15" name="answertext" id="answertext" ></textarea><a class="xmlbutton buttonblue btn-outlined" name="editorLoaderBtn" id="editorLoaderBtn" onclick="loadWYSIBB()"><span class="icon-wysiwyg"></span> Muat Editor</a><div id="btn_uploadFileCont" style="display:none"><a onclick="window.open(\'tce_select_userphoto.php?frm=testform&amp;fld=answertext,\',\'mediaselect\',\'height=600,width=680,resizable=yes,menubar=no,scrollbars=yes,toolbar=no,status=no,modal=yes\');" id="btn_uploadFile" class="xmlbutton buttonblue"><span class="icon-info"></span> upload file / Sisipkan media</a>&nbsp;<a id="imgProblem" href="#imgProblem">Apabila gambar tidak tampil pada editor, silakan klik disini</a></div>';
            } else {
                ...
                if ($ra = F_db_query($sqla, $db)) {
                    while ($ma = F_db_fetch_array($ra)) {
                        ...
                        if ($m['question_type'] == 4) {
							...
							$qblock_arr['content'] .= '<input checked="checked" type="radio" name="answpos" id="answpos_'.$i.'" value="'.$i.'"/>&nbsp;<label for="answpos_'.$i.'">';
                        } elseif ($m['question_type'] == 1) {
                            // MCSA
                            // $ret .= '<input type="radio" name="answpos" id="answpos_"'.$i.' value="1"/>&nbsp;<label for="answpos_"'.$i.'>';
                            $qblock_arr['content'] .= '<input type="radio" name="answpos" id="answpos_'.$i.'" value="'.$i.'"/>&nbsp;<label for="answpos_'.$i.'">';
                        ...
                        if ($m['question_type'] == 2) {
							// $ret .= '<input type="checkbox" name="answpos['.$i.']" id="answpos_'.$i.'" value="1">&nbsp;<label for="answpos_'.$i.'">';
							$qblock_arr['content'] .= '<input type="checkbox" name="answpos['.$i.']" id="answpos_'.$i.'" value="1">&nbsp;<label for="answpos_'.$i.'">';
						}
                        if ($m['question_type'] == 4) {
							$max_position = F_count_rows(K_TABLE_LOG_ANSWER, 'WHERE logansw_testlog_id='.$m['testlog_id'].'');
							// $ret .= '<select name="answpos['.$i.']" id="answpos_'.$i.'" size="0">';
							$qblock_arr['content'] .= '<select name="answpos['.$i.']" id="answpos_'.$i.'" size="0">';
							if (F_getBoolean($test_data['test_noanswer_enabled'])) {
                                    // $ret .= '<option value="0">&nbsp;</option>'.K_NEWLINE;
                                    $qblock_arr['content'] .= '<option value="0">&nbsp;</option>'.K_NEWLINE;
                                }
								
                            for ($x=1; $x <= $max_position; $x++) {
								// $ret .= '<option value="'.$x.'"';
								$qblock_arr['content'] .= '<option value="'.$x.'"';
								if ($x == $ma['logansw_position']) {
									// $ret .= ' selected="selected"';
									$qblock_arr['content'] .= ' selected="selected"';
								}
								// $ret .= '>'.$x.'</option>'.K_NEWLINE;
								$qblock_arr['content'] .= '>'.$x.'</option>'.K_NEWLINE;
							}
							// $ret .= '</select>'.K_NEWLINE;
							$qblock_arr['content'] .= '</select>'.K_NEWLINE;
							// $ret .= '<label for="answpos_'.$i.'">';
							$qblock_arr['content'] .= '<label for="answpos_'.$i.'">';
				...
				if ($m['question_type'] == 1) {
					// $qblock_arr['content'] .= '<li'.$noanswer_hidden.'><input type="radio"'.$noanswer_disabled.' name="answpos" id="answpos_0" value="0" checked="checked"><label for="answpos_0">'.$l['m_unanswered'].'</label></li>'.K_NEWLINE;
					$qblock_arr['content'] .= '<li'.$noanswer_hidden.'><input type="radio"'.$noanswer_disabled.' name="answpos" id="answpos_0" value="0"><label for="answpos_0">&nbsp;'.$l['m_unanswered'].'</label></li>'.K_NEWLINE;
				}
			...
			if(!file_exists(K_PATH_QBLOCK.$m['testlog_id'].'.json')){
				$fp = fopen(K_PATH_QBLOCK.$m['testlog_id'].'.json', 'w');
				fwrite($fp, json_encode($qblock_arr, JSON_UNESCAPED_UNICODE));
				// fwrite($fp, json_encode($qblock_arr));
				fclose($fp);
				echo 'localStorage.setItem("json_'.$m['testlog_id'].'","1");'.K_NEWLINE;
				
				if(TRIGSS_CACHE_ALL){
					// echo 'arrTestLogIDfromServer['.$a.'] = '.$m['testlog_id'].';'.K_NEWLINE;
					echo 'if(localStorage.getItem("cacheEnable")==="1"){'.K_NEWLINE;
					echo 'arrTestLogIDfromServer['.$a.'] = "../../qblock/'.$m['testlog_id'].'.json";'.K_NEWLINE;
					echo '}'.K_NEWLINE;
		...
		if(TRIGSS_CACHE_ALL){
			...
			/* echo 'for (let i=0; i <= arrTestLogIDfromServer.length-1; i++) {'.K_NEWLINE; 
			echo '   testlogTask(i);'.K_NEWLINE;	   
			echo '}'.K_NEWLINE; 
			  
			echo 'function testlogTask(i) { '.K_NEWLINE;
			echo '  setTimeout(function() { '.K_NEWLINE;
			echo '	  caches.open(\'qBlockCache\').then(cache =>{cache.add(\'../../qblock/\'+arrTestLogIDfromServer[i]+\'.json\');});'.K_NEWLINE;
			echo '  }, 500 * i); '.K_NEWLINE;
			echo '}'.K_NEWLINE; */
			echo 'if(localStorage.getItem("cacheEnable")==="1"){'.K_NEWLINE;
			echo '	  caches.open(\'qBlockCache\').then(cache =>{cache.addAll(arrTestLogIDfromServer);});'.K_NEWLINE;
			echo '}'.K_NEWLINE;
		}
