...
( function() {
	...
	CKEDITOR.cleanWord = function( mswordHtml, editor ) {
		...
		mswordHtml = mswordHtml.replace( /<m:omath>.*<\/m:omath>/g, '' );
		mswordHtml = mswordHtml.replace( /<m:r>.*<\/m:r>/g, '' );
		mswordHtml = mswordHtml.replace( /<m:omathpara>.*<\/m:omathpara>/g, '' );
		mswordHtml = mswordHtml.replace( /<m:omathparapr>.*<\/m:omathparapr>/g, '' );
		mswordHtml = mswordHtml.replace( /<m:ssup>.*<\/m:ssup>/g, '' );
			...
			elements: {
				...
				'img': function( element ) {
					...
					if ( element.attributes.src && element.attributes.src.match( /^file:\/\// ) &&
						...
						element.attributes.alt = 'Apabila gambar tidak terlihat, coba reload halaman';
						//alert(element.attributes.src);
					}
					
					if ( (element.attributes.src && element.attributes.src.match( /^file:\/\// )) || (element.attributes.src && element.attributes.src.match( /^http:\/\// ))) {
						element.attributes.alt = 'Apabila gambar tidak terlihat, coba reload halaman';
						//element.attributes.src = element.attributes.alt;
						//alert(element.attributes.src);
					}else{
						//alert("xxx xxx xxx");
						element.attributes.src = '../../cache/upload/'+element.attributes.src;
						element.attributes.alt = 'Apabila gambar tidak terlihat, coba reload halaman';
					}
