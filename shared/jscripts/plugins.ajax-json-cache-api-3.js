...
function loadQuestion(a,b){
	...
	if(localStorage.getItem("json_"+testlogid) && window.location.protocol==="https:" && b.id!=="refbtn"){
		// alert("xxx");
	...
	// if(false){
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 404){
				if(b.id==="refbtn"){
					var qblockRefresh = '&qblock-refresh=1';
				}else{
					var qblockRefresh = '';
				}
				xhttp.open("POST", "tce_test_execute.php", true);
				...
				xhttp.send("question-block&testid="+testid+"&testlogid="+testlogid+"&testuser_id="+testuser_id+"&examtime="+examtime+"&prevquestionid="+prevquestionid+"&nextquestionid="+nextquestionid+"&autonext="+autonext+"&finish="+finish+"&display_time="+display_time+"&reaction_time="+reaction_time+"&csrf_token="+csrf_token+qblockRefresh);
			}
			if (this.readyState == 1){
				if(b!==0){
