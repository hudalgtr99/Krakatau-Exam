...
// var formChanged;
function qBlockRender(res){
	var questionBlockOffline = JSON.parse(res);
	var asd = questionBlockOffline.qdesc;
	asd += "<div class='rowl'>";
	if(questionBlockOffline.qtype==="1"){
		asd += "<ol class='answer'>";
		var ansDescArr = questionBlockOffline.ansdesc;
		for (x in ansDescArr) {
			if(x!=="0"){
				asd += "<li><input type='radio' name='answpos' id='answpos_"+x+"' value='"+x+"' /><label for='answpos_"+x+"'>"+ansDescArr[x]+"</label></li>";						
			}
		}
		asd += "<li><input type='radio' name='answpos' id='answpos_0' value='0' /><label for='answpos_0'>"+ansDescArr[0]+"</label></li>";
	}
	asd += "</div>";
	document.getElementById("question-area").innerHTML = asd;
	// console.log(questionBlockOffline.checked);
	document.getElementById("answpos_"+questionBlockOffline.checked).setAttribute("checked","checked");
}

function loadQuestion(a,b){
	//alert(b);
	// window.formChanged = DetectChanges("testform");
if(fTestForm){
	...
	var testlogid_exist = localStorage.getItem(testLogId.value);
	if(testlogid_exist)
	{
		// load from localStorage
		// document.getElementById("question-area").textContent = testlogid_exist;
		qBlockRender(testlogid_exist);
		//
		if(a!==1000){
			dynNoSoal(a);
		}else{
		...
		xhttp.onreadystatechange = function(){
			...
			if (this.readyState == 4 && this.status == 200){
				...
				if(this.responseText.length>0){
					...
					testlogid_exist = localStorage.getItem(questionBlockData.testlogid);
					if(!testlogid_exist){
						localStorage.setItem(questionBlockData.testlogid,this.responseText);
						qBlockRender(this.responseText);
					}			
				}else{
